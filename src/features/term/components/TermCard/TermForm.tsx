...
const TermForm: FC<{ form: UseFormReturn<any, any, any>, remove?: UseFieldArrayRemove, prefix: string }> = ({ form: { control, getValues, setValue, }, prefix }) => {
    ...
    const wrappedTtranslate = async ({ term, definition }: TermInput) => {
        if (definition.lang && term.lang && term.content && (term.lang !== definition.lang)) {
            ...
            if (res.success) setOptions(res.data.translations);
        }
    }
    ...
    const change = async (value: string, section: "term" | "definition") => {
        ...
        let lang = ""
        const res = await langDetection(value);

        if (res.success) {
            setValue(`${prefix}${section}.lang`, res.data.lang)
            lang = res.data.lang
        }

        wrappedTtranslate({ ...state, [section]: { lang, content: section === "term" ? value : [value] } })
    }
    ...
    return (
        <Stack sx={{
            ...
            <Stack sx={{ gap: 0.5, flexBasis: 1, flexGrow: 1 }}>
                <Controller control={control} name={`${prefix}definition.content`} render={
                    ({ field: { onChange, value } }) => (
                        <Autocomplete
                            size="small"
                            multiple
            ...
            <Controller control={control} name={`${prefix}exampleSentence`} render={
                ({ field }) => (
                    <TextField
                        {...field}
                        multiline
