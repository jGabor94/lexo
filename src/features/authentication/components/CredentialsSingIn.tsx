...
const CredentialsSingIn: FC<{ redirect?: string }> = ({ redirect }) => {
    ...
    const onSubmit = handleSubmit(async ({ email, password }) => {
        setLoading(true)
        const res = await signIn("credentials", { email, password, ...redirectPath ? { redirectTo: redirectPath } : { redirect: false } })
        if (res.error) {
