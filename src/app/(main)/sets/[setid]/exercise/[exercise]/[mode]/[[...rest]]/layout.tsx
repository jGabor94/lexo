...
const layout: FC<{
    children: ReactNode,
    params: Promise<{ setid: string, exercise: string, mode: string, rest?: string[] }>,
}> = async ({ children, params }) => {
    const { setid, exercise: exerciseParam, mode: modeParam, rest } = await params;
    const taskid = rest ? rest[1] : undefined
    const exercise = exerciseParam as Exercise;
    const mode = modeParam as ExerciseMode;

    if (!exerciseTheme[exercise] || (rest && (rest.length !== 2 || rest[0] !== "task"))) return notFound()
    const res = await getSet(setid, taskid)
    if (!res.success) return <>Hiba {res.error.type}</>
    const { data: set } = res
    const isOwner = await getIsOwner(set.user.id)
    if (!isOwner && !set.task && mode !== "free") redirect(`/sets/${set.id}/exercise/${exercise}/free`)
            ...
            <Stack gap={3} sx={{ overflowX: "visible", }} >
                <Paper component={Stack} direction="row" p={2} justifyContent="space-between" alignItems="center" gap={2} sx={{ border: "none", backgroundColor: "background.paper" }} >
                    ...
                    <Tooltip title="Bezárás" sx={{ height: "max-content" }} >
                        <Link href={`/sets/${set.id}${taskid ? `/task/${taskid}` : ''}`} style={{ textDecoration: "none", color: "inherit" }}>
                            <X />
                        </Link>
                    </Tooltip>
